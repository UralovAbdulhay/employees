package com.example.demo;

public class Urls {
    public static final String EMPLOYEE_QUE = "employee";
    public static final String EMPLOYEE_SAVE = "employee";
    public static final String EMPLOYEE_UPDATE = "employee_update";

    public static final String ATTENDANCE_QUE = "attendance";
    public static final String ATTENDANCE_SAVE = "attendance_save";
    public static final String ATTENDANCE_UPDATE = "attendance_update";

    public static final String DEPARTMENT_QUE = "department";
    public static final String DEPARTMENT_SAVE = "department_save";
    public static final String DEPARTMENT_UPDATE = "department_update";

    public static final String POSITION_QUE = "position";
    public static final String POSITION_SAVE = "position_save";
    public static final String POSITION_UPDATE = "position_update";

    public static final String TOPIC_EXCHANGE = "topic_exchange";


}
